...
直接统计全局的cache miss率(100次访问cache,有1.348次会miss)
...
这里有个有意思的事是，当没有运行demo程序时，系统全局由于访问cache总数不多，但是miss率却达到3.716%,所以不能光看比率,还要比总数
...
编译一个使能cache line aligned的程序再测试,发现cache miss率下降
...
## 在x86上的测试(Test on x86 linux ubuntu 22.04)
